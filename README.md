...
网易云音乐（`com.netease.cloudmusic`，实测 9.5.96 / 9.6.0）的本机 LSPosed 模块：协议观测 + 界面整理 + 消息防撤回台账。
...
- **日志分级**：主日志在宿主私有目录（`/data/data/<pkg>/files/cm_hook.log`），大内容截断、
  过滤日志仅在实际改动时记录；LSPosed 日志仅镜像装载与操作事件，不再刷爆日志缓冲区
...
- **首页推荐精细控制**：顶部卡片行整行隐藏 / 行内单卡隐藏 / 内容区块隐藏（勾选=隐藏），
  块清单按当前账号从响应自动发现、直接显示页面文案（如「猜你喜欢的「华语」好歌」「xx的雷达歌单」）；
  隐藏集改动自动刷新 MMKV 块缓存，精细页内置「重启网易云生效(冷启动)」按钮
...
- 模块只在**本机进程内**读取/改写目标 App 自身的网络请求与本地缓存：**不提供服务端能力、不绕过付费或版权内容、不修改服务端数据、不上传或收集任何账号与内容**。日志与台账仅落在本机 App 私有目录（`/data/data/com.netease.cloudmusic/files/`）。
...
- 实测目标版本 9.5.96(9005096) / 9.6.0(9006000)。模块内置 DexKit 兜底与自检日志（宿主私有目录 `cm_hook.log`）。
